...
public class EnemyController2 : MonoBehaviour
{
	...
	private GridUnit enemyGridScript; // script attached to enemyGrid
	...
	void FixedUpdate ()
	{
		...
		if (enemyGridScript) {
			Vector2 curGridCoord = new Vector2 (enemyGridScript.col, enemyGridScript.row);
		...
		if (grounded) {
			if (moveVector.y > 0 && moveVector.x != 0) { // if next point is diagonal up
				...
				nextGridCoord = new Vector2 (enemyGridScript.col + dir, enemyGridScript.row);
				Move (dir);
			} else {
				// move in x toward next point
				Move (dir);
	...
	void FindNewPath ()
	{
		...
		PathFind.Point _from = new PathFind.Point (enemyGridScript.col, enemyGridScript.row);
		...
		if (newList.Count > 0) {
			...
			nextGridCoord = new Vector2 (pathToPlayer [pathIndex].x, pathToPlayer [pathIndex].y);
			pathIndex = 0;
	...
	void OnTriggerEnter2D (Collider2D col)
	{
		...
		Debug.Log (string.Format("Enemy currently in {0}", enemyGrid.name));
		enemyGridScript = enemyGrid.GetComponent<GridUnit> ();
		if (enemyGridScript.canNavigateTo) {
			pathIndex++;
			if (pathIndex >= pathToPlayer.Count) {
				pathIndex = pathToPlayer.Count;
			}
			Debug.Log (string.Format ("pathIndex: {0}", pathIndex));
			nextGridCoord = new Vector2 (enemyGridScript.col, enemyGridScript.row);
		}
